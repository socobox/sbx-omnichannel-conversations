...
| `authorType` | `string \| undefined` | **Nuevo.** El `participant_type` (`"USER"`, `"HUMAN_AGENT"`, `"AI_AGENT"`, ...) de quien mandó el mensaje — por ejemplo para distinguir un mensaje de bot sin pedir `getParticipants()` aparte y cruzarlo a mano contra `author`. Mismo patrón GETTER que `authorName` (se resuelve solo si el participante llega después, vía el mismo refetch en segundo plano). `undefined` (no `null`) cuando el participante todavía es desconocido — a diferencia de `authorName`, no hay un estado real de "participante conocido, sin tipo", así que no hace falta reservar `null` para eso. |
